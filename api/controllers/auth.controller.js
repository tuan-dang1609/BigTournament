...
export const signup = async (req, res, next) => {
  const { nickname,riotID, username, password, discordID } = req.body;
  try {
...
export const leagueHOF = async (req, res, next) => {
  try {
    ...
    const operations = leagues.map((league) => ({
      updateOne: {
        filter: { id: league.id,game:league.game,name:league.name }, // Điều kiện xác định giải đấu đã tồn tại
        update: { $set: league },
        upsert: true // Thêm mới nếu không tồn tại
      }
    }));

    ...
    aov: [
      {
        ...
        current: location.pathname === '/tft/ranking/total/day1',
      },
      {
        name: 'Vòng 2',
        href: '/tft/ranking/total/day2',
        current: location.pathname === '/tft/ranking/total/day2',
      },
      {
        name: 'Chung kết tổng',
        href: '/tft/grandfinal',
        current: location.pathname === '/tft/grandfinal',
      },
    ],
  };
  ...
  const pointsDisplay = score.map((item) => {
    return (
      <p className="text-white">
        <strong className="text-[#ff7104]">Hạng {item.top}</strong> → {item.point} Điểm
      </p>
    );
  });
  useEffect(() => {
      ...
      try {
        ...
        for (const lobby of lobbies[day]) {
          ...
          puuids.forEach((puuid, index) => {
            const totalPoints = lobbyParticipants[puuid].points.reduce(
              (acc, curr) => acc + curr,
              0
            );
      ...
      <div className="mt-40 mx-auto lg:w-[85%] w-[92%]">
        <div className="flex lg:flex-row flex-col-reverse items-center justify-center">
          <div className="lg:w-[70%] w-full">
            <h2 className="uppercase p-2 text-base-content font-bold lg:text-[32px] text-[30px] lg:text-left text-center">
              Bảng xếp hạng tổng - {day.toUpperCase()}
            </h2>
            <h2 className="uppercase p-2 text-center lg:text-[25px] text-[18px] text-base-content font-bold">
              Cách tính điểm
            </h2>
            ...
            <button
              onClick={startTour}
              className="bg-[#48042c] font-bold text-white px-4 py-2 rounded-md mt-2 mb-4"
            >
              Luật Tiebreaker
            </button>
          </div>
          ...
          <div className="overflow-x-auto rounded-es-lg rounded-ee-lg shadow-md border border-[#48042c] mb-10 ">
            <div className="flex lg:flex-row flex-col-reverse items-center justify-center ">
              <table
                ...
                cellPadding="10"
                cellSpacing="0"
                className="table-fixed min-w-full text-base-content lg:w-full w-[140%] font-semibold"
              >
                <thead className="bg-[#48042c] text-white">
                  <tr>
                    <th className="p-2 bg-[#48042c] sticky left-0 z-10 lg:!w-[270px] !w-[180px]">
                      Game Name
                    </th>

    ...
    const fetchData = async () => {
      try {
        const allTeamResponse = await axios.post(
          'https://bigtournament-1.onrender.com/api/auth/findallteamTFTDouble'
        );
        ...
        const rankedTeams = await Promise.all(
          allTeams.map(async (team) => {
            const rankResponse = await axios.post(
              'https://bigtournament-1.onrender.com/api/auth/tft_double_rank',
              team
            );
      ...
      {loading ? (
        ...
        <>
          <div className="lg:mt-32 mt-24 flex lg:flex-row flex-col lg:gap-y-1 gap-y-2 items-center justify-center mb-2 gap-x-2">
            ...
            <div className="badge badge-primary badge-outline p-2 mt-1 font-bold">BETA</div>
          </div>
          <table
            border="1"
            cellPadding="4"
                ...
                <tr key={team._id}>
                  ...
                  <td className="text-left ">
                    <div className="flex flex-row">
                      ...
                      />
                      <p className="flex items-center lg:text-[15px] text-[12px]">
                        {team.teamName}
                      </p>
                    </div>
                  </td>
                        ...
                        <>
                          <p className="flex items-center w-full lg:text-[15px] font-semibold text-[12px]">
                            {team.averageRank.losses + team.averageRank.wins === 0
                              ? 0
                              : (
                                  (team.averageRank.wins * 100) /
                                  (team.averageRank.losses + team.averageRank.wins)
                                ).toFixed(1)}{' '}
                            %
                          </p>
                        </>
                      ...
                      {team.averageRank ? (
                        <div className="flex lg:flex-row flex-col mx-auto">
                          ...
                          <p className="flex items-center lg:text-[15px] font-semibold text-[10.5px] lg:w-[180px]">
                            {team.averageRank.tier} {team.averageRank.rank}{' '}
                            {team.averageRank?.leaguePoints || ''} LP
                          </p>
                        </div>
                      ) : (
                        'N/A'

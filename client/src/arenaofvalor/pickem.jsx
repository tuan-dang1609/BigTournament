...
const PickemChallenge = () => {
  ...
  const handleTeamSelection = (team) => {
    ...
    if (currentQuestion.maxChoose === 1) {
      ...
      setTempSelection((prevSelection) => (prevSelection[0] === team.name ? [] : [team.name]));
    } else {
                ...
                >
                  ...
                  <div className="flex items-center justify-center">
                    {predictions[question.id]?.length > 0 ? (
                      <div
                        className={`px-3 grid grid-cols-${question.maxChoose} gap-x-8 items-center`}
                      >
                        {predictions[question.id].map((team) => {
                          const selectedTeam = question.options.find(
                          ...
                          return selectedTeam ? (
                            <div key={team} className="flex flex-col items-center">
                              ...
                              <p className="text-center text-[14px] mt-1 font-semibold">
                                {selectedTeam.name}
                              </p>
                            </div>
                          ) : null;
              ...
              {questionsGroup2.map((question) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-center">
                    {predictions[question.id]?.length > 0 ? (
                      <div
                        className={`px-3 grid grid-cols-${question.maxChoose} lg:gap-x-20 sm:gap-x-12 gap-x-10  items-center`}
                      >
                        {predictions[question.id].map((team) => {
                          const selectedTeam = question.options.find(
                            (option) => option.name === team
        ...
        {tempSelection.length === currentQuestion?.maxChoose && (
          <div className="mt-6 flex justify-end">
            <button
              ...
              className="bg-accent text-white px-2 py-3 rounded-lg"
            >
              Xác nhận & Gửi
            </button>
          </div>

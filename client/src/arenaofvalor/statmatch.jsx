...
export default function MatchStat() {
  ...
  useEffect(() => {
    if (selectedMap?.infoTeamleft?.data) {
      const totalGoldA = selectedMap.infoTeamleft.data.reduce(
        (sum, player) => sum + (parseFloat(player.Gold) || 0),
        0
      );
      const totalGoldB = selectedMap.infoTeamright.data.reduce(
        (sum, player) => sum + (parseFloat(player.Gold) || 0),
        0
      );
  ...
  const { teamA, teamB, scoreteamA, scoreteamB, timestartmatch, league, type, group } =
    matchData2?.[0] || {};

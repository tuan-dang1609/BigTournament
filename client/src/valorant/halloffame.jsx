...
const TeamPageHOF = () => {
  ...
  const [selectedLeague, setSelectedLeague] = useState(
    () => localStorage.getItem('selectedLeague') || 'season1'
  );
        ...
        <>
          ...
          <div className="lg:w-[80%] py-12 px-4 sm:px-6 lg:px-8 lg:mt-0 md:mt-10 lg:ml-[20%] w-full">
            <div className="max-w-[1400px] mx-auto">
              <h1 className="text-4xl font-bold text-center mb-12 mt-16 md:mt-0">
                Sảnh danh vọng các đội
              </h1>
                ...
                {teamsData.length > 0
                  ? teamsData.map((team) => (
                      <React.Fragment key={team._id || team.name}>
                        <TeamCard team={team} />
                        <div
                          key={`divider-${team._id || team.name}`}
                          className="border-t border-gray-300 my-6 last:border-none"
                        ></div>
                      </React.Fragment>
                    ))

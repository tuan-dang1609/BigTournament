...
const TeamRegistrationForm = () => {
    ...
    const fetchRiotIDSuggestions = async (query) => {
        if (!query) {
            setSuggestions([]);
            return;
        }
        ...
        try {
            ...
            // Lọc dữ liệu theo query (tìm kiếm trong username hoặc riotId)
            const filteredUsers = users.filter(user =>
                user.username.toLowerCase().includes(query.toLowerCase()) ||
                user.riotId.toLowerCase().includes(query.toLowerCase())
            );

            setSuggestions(filteredUsers);
        } catch (error) {
            console.error("Lỗi khi lấy gợi ý RiotID:", error);
            setSuggestions([]);
        }
    };
                ...
                <div className="min-h-screen flex flex-col sm:mx-96 mx-5 ">
                    ...
                    <div className="bg-white p-8 rounded-lg shadow-md w-full flex justify-center items-center flex-col">
                        ...
                        <div className="text-gray-600">
                            <p>Thành viên Valorant:</p>
                            {userRegister.gameMembers["Valorant"].map((member, index) => (
                                <div key={index} className="text-center text-gray-600">
                                    <strong>RiotID của thành viên {index + 1}:</strong> <span>{member}</span>
                                </div>
                            ))}
                        </div>
